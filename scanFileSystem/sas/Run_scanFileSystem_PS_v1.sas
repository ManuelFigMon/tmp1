...
%macro scanFileSystemPS(
    ...
    -----------------------------------------------------------------*/
    %if not %sysfunc(fileexist(%superq(PS_SCRIPT_PATH))) %then %do;
        %put ERROR: Script not found: %superq(PS_SCRIPT_PATH);
        %put ERROR- Fix the PS_SCRIPT_PATH macro variable at the top of this file.;
        %abort cancel 8;
    %end;

    %let _log = %sysfunc(pathname(work))\scanps_output.log;
    ...
    -----------------------------------------------------------------*/
    %put NOTE: ---- scanFileSystem.ps1 output ----;
