<#
=====================================================================
  Program Name  : formatData.ps1
  Author        : Manuel Figallo
  Purpose       : Convert a CSV *or an Excel workbook* into a styled Excel
                  workbook with a SAS ODS-style look and feel.
  Version       : 1.2beta
  Created       : 2026-08-26
  Last Modified : 2026-09-06

  Dependencies:
    NONE that must be installed. ImportExcel is USED WHEN PRESENT for
    WRITING because it is well tested; when it is absent the script writes
    the .xlsx itself using only .NET types that ship with Windows
    PowerShell. READING an .xlsx never needs the module.

  Description:
    PowerShell twin of src/py/formatData.py. The default "corporate" style is
    a navy title banner, blue header row and zebra striping.

    Renamed from formatCSV, which now accepts an .xlsx as well as a .csv --
    "CSV" had stopped describing what it does. formatCSV.ps1 remains beside
    this file and forwards to it, so a caller copied to the share before the
    rename keeps working.

    FEEDING BACK ITS OWN OUTPUT. A workbook this script produces has the
    title banner on row 1 and the headers on row 2. Reading one back detects
    that automatically, so one call's output can be the next call's input
    with no extra parameter. -HeaderRow overrides the detection.
  ...
  Input Parameters (required first):
    ...
    -SheetName (default Report)   -- the OUTPUT worksheet
    -Title (defaults to the input file name)
    -InputSheet -- which worksheet to read from an .xlsx input; default first
    -HeaderRow  -- 1-based header row in an .xlsx input; 0 = detect
  ...
  Change Log:
    ...
    v1.2beta - Renamed from formatCSV; accepts an .xlsx input as well as a
               .csv, read natively via Read-CgsXlsxSheet.
=====================================================================
#>
[CmdletBinding()]
param(
...
function Invoke-Main {
    <# .SYNOPSIS Style the input into a workbook. .OUTPUTS [int] exit code. #>
    $source = if ($InputPath) { $InputPath } else { $InputCsvPath }
    if (-not $source)          { Write-CgsError "required parameter 'InputPath' is missing or empty"; return 2 }
    ...
    if (-not (Test-Path -LiteralPath $source)) {
        Write-CgsError "input file not found: $source"
        return 3
    }

    # Read the input, whichever of the two shapes it is. Both paths produce
    # objects with the same properties, so nothing below this cares.
    ...
    $banner  = if ($Title) { $Title } else { [System.IO.Path]::GetFileNameWithoutExtension($source) }
